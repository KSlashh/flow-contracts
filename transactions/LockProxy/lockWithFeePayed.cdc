...
import FeeManager from "../../contracts/FeeManager.cdc"
...
transaction(
    ...
    prepare(acct: AuthAccount) {
        ...
        // pay fee
        var txIndex = CrossChainManager.EthToPolyTxHashIndex - 1
        let feeVaultStoragePath: StoragePath = (feeVaultPath as? StoragePath)!
        let feeVaultRef = acct.borrow<&FungibleToken.Vault>(from: feeVaultStoragePath)
            ?? panic("Could not borrow a reference to the owner's fee vault")
        let fee <- feeVaultRef.withdraw(amount: feeAmount)
        FeeManager.payFee(tag: txIndex.toString(), fund: <-fee)
    }
}